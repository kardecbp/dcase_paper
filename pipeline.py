...
import pandas as pd
import librosa
from metrics import mapk, Mapk_callback
from keras.callbacks import EarlyStopping, LearningRateScheduler, ModelCheckpoint, ReduceLROnPlateau
from keras.utils import to_categorical
from augmentation import get_random_eraser, MixupGenerator
from sklearn.model_selection import StratifiedKFold
from keras.preprocessing.image import ImageDataGenerator
from models import mixed_cnn, mini_domain, mini_domain2, domain_model
import pickle
...
def load_data(df, config, data_dir):
	...
	for i, fname in enumerate(df.index):
		...
		if config.transform == 'cqt':
			data = librosa.core.cqt(data, sr=config.sampling_rate, n_bins=config.n_coef)
		elif config.transform == 'melspec':
			data = librosa.feature.melspectrogram(data, sr=config.sampling_rate, n_mels=config.n_coef) 
		all_data.append(data)
		
	return all_data


def create_dataset(X, y, config, mode='train'):
	# spliting longer files and creating dataset, original ids are saved in true_ids list
	X_fin, true_ids, y_fin = [], [], []
	for i in range(len(X)):
		if X[i].shape[1] == config.spec_len:
			example = librosa.amplitude_to_db(X[i], ref=np.max)
			X_fin.append(example), true_ids.append(i)
			if mode == 'train':
				y_fin.append(y[i])
		else:
			start =  0
			while True:
				if (start+config.spec_len) <  X[i].shape[1]:
					...
					start += config.step
					example = librosa.amplitude_to_db(example, ref=np.max)
					X_fin.append(example), true_ids.append(i)
					if mode == 'train':
						 y_fin.append(y[i])	
				else:
...
def preprocess(config):
	...
	X, y, true_ids = create_dataset(X, y, config)
...
def evaluate(preds, ids):
	# generating sample prediction by averaging all predictions with same id
	real_preds = {}
...
def train(model_name, config, model_id=1):
	...
	X_train, y_train, X_val, y_val, label_ids, ids_train, ids_val = preprocess(config)
	...
	X_test, _, test_ids = create_dataset(X_test, y=None, config=config, mode='test')
...
if __name__ == '__main__':
	# configs = [Config(transform='melspec'), Config(transform='cqt', n_coef=110, audio_duration=5, shift_duration=4)]
	# models = ['model1_melspec_4s', 'model1_cqt_5s_110']
	'''
	configs = [Config(transform='cqt', n_coef=110, audio_duration=8, shift_duration=6, batch_size=16),
				Config(transform='melspec', audio_duration=6, shift_duration=5)]
	configs = [Config(transform='melspec', audio_duration=6, shift_duration=5)]
	# models = ['model1_cqt_8s_110', 'model1_melspec_6s']
	models = ['model1_melspec_6s']
	
	configs = [Config(transform='cqt', batch_size=32), Config(transform='cqt', batch_size=32)]
	models = ['model3_cqt_4s', 'model4_cqt_4s']
	model_ids = [3, 4]
	for i in range(len(models)):
		train(models[i], configs[i], model_ids[i])
	'''
	X = pickle.load(open('X_train_cqt4s_24kHZ.pickle', 'rb'))
	print(X.shape)
